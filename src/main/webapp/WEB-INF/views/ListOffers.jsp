    ...
    <script>
        ...
        function filterByDiscount(discount) {
            document.querySelectorAll('.discount-btn').forEach(btn => {
                btn.classList.remove('active');
                if (btn.dataset.discount === discount) {
                    btn.classList.add('active');
                }
            });

            filterOffers(); // Call global filter with discount too
        }

        // Global filter by dropdowns + discount
        function filterOffers() {
            const selectedCity = document.getElementById('city-select').value.toLowerCase();
            const selectedArea = document.getElementById('area-select').value.toLowerCase();
            const selectedOfferType = document.getElementById('offer-select').value.toLowerCase();
            const selectedDiscount = document.querySelector('.discount-btn.active').dataset.discount;

            document.querySelectorAll('.offer-card').forEach(card => {
                const city = card.dataset.city.toLowerCase();
                const area = card.dataset.area.toLowerCase();
                const offerType = card.dataset.offertype.toLowerCase();
                const discount = card.dataset.discount;

                const cityMatch = !selectedCity || city.includes(selectedCity);
                const areaMatch = !selectedArea || area.includes(selectedArea);
                const typeMatch = !selectedOfferType || offerType.includes(selectedOfferType);
                const discountMatch = selectedDiscount === 'all' || discount === selectedDiscount;

                if (cityMatch && areaMatch && typeMatch && discountMatch) {
                    card.style.display = 'block';
                } else {
                    card.style.display = 'none';
                }
